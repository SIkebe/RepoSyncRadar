...
public class StartupSignInTests
{
    ...
    [Fact]
    public async Task ConfigureAppConfiguration_When_Default_User_Local_Directory_Missing_Does_Not_Throw()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), "rsr-startup-config-tests-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var basePath = Path.Combine(tempRoot, "app");
        ...
        try
        {
            ...
            Assert.False(configuration.GetValue<bool>("Updates:Enabled"));
            Assert.True(Directory.Exists(Path.Combine(localAppData, "RepoSyncRadar")));
        }
        finally
        {
            try
