...
public sealed class AppHeaderTests
{
    ...
    public void Settings_Shows_Empty_State_When_No_Boost_Rules_Exist()
    {
        ...
        cut.WaitForAssertion(() =>
        {
            ...
            Assert.Empty(cut.FindAll("[data-testid=\"settings-boost-rule-registration\"]"));
            Assert.Empty(cut.FindAll("[data-testid=\"settings-add-boost-rule\"]"));
        });
    }
    ...
    public void Header_Dismisses_Background_Update_Failure_Until_Activity_Changes()
    {
        ...
        var secondFailure = new AppUpdateActivity(
            AppUpdateActivityStatus.Failed,
            Message: "rate limit exceeded");
        updateService.Publish(secondFailure);
        ...
        var thirdFailure = new AppUpdateActivity(
            AppUpdateActivityStatus.Failed,
            Message: "rate limit exceeded");
        updateService.Publish(thirdFailure);

        cut.InvokeAsync(() => cut.Instance.DismissFailedUpdate(secondFailure));
