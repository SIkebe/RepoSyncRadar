...
- **Commit List**: 取り込まれたコミットの一覧。クリックで詳細へ。差分解説が保存済みの行には「解説済み」と表示します。
...
- 共有文案は選択中のモデルで SDK の型付き構造化出力を使用します。JSON Schema に対応したモデルを選んでください。構造化出力が失敗した場合は文案を保存せず、エラーを表示します。
