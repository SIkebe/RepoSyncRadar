...
設定パネルの **Copilot 使用量** では、SDK の usage event / session metrics が返す AI Credits と Premium Request cost を表示します。SDK が AI Credits を返さない場合、アプリ側でモデル別価格から推定せず `credits 未報告` のままにします。`GitHub.Copilot.SDK` 1.0.5-preview.1 の `Usage.GetMetricsAsync()` から `TotalNanoAiu` とモデル別 `TotalNanoAiu` を取得します。
