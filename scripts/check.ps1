...
.DESCRIPTION
    `dotnet build -warnaserror` と `dotnet test --filter "Category!=Manual"` を順に走らせ、
...
try {
    ...
    Write-Host '==> dotnet test --no-build --filter "Category!=Manual"' -ForegroundColor Cyan
    dotnet test --no-build --configuration $Configuration --filter 'Category!=Manual'
    if ($LASTEXITCODE -ne 0) {
