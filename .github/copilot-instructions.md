...
- Synchronized preview scrolling must refresh its local `lastScrollTop` after every programmatic peer scroll. Disable browser scroll anchoring, apply identical wrapping to corresponding code blocks before measuring gaps, and reconcile both panes to the lower post-layout `scrollTop`; otherwise resize reflow or clamping can leave the panes at different positions. Initial and user-requested diff navigation may calculate target centers independently, but must reconcile both panes to one absolute `scrollTop` before committing the navigation. Diff navigation overlays must not extend vertically beyond their targets into adjacent callouts.
