...
- `GitHub.Copilot.SDK` 1.0.7-preview.3 nupkg requests `CopilotCliVersion=1.0.71-2`. That CLI is published as official `github/copilot-cli` GitHub Release assets rather than npm platform packages, so `Directory.Build.targets` and `scripts/Build-VelopackRelease.ps1` download the matching Windows `.zip` and pass its executable through `CopilotCliBinaryPath`.
