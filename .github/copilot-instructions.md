...
- Synchronized preview scrolling must refresh its local `lastScrollTop` after every programmatic peer scroll. Disable browser scroll anchoring while alignment gaps are active, and reconcile both panes to the lower post-layout `scrollTop`; otherwise resize reflow or clamping can leave the panes at different positions.
