...
RepoSyncRadar is a Windows desktop app for monitoring `github/docs` Repo sync PRs, triaging important documentation changes with the GitHub Copilot SDK, previewing rendered docs changes, and generating sharing drafts for Twitter and customer-facing notices. The solution is C#/.NET with WPF, BlazorWebView, MudBlazor, WebView2, EF Core SQLite, Octokit, and `GitHub.Copilot.SDK` 1.0.6.
...
- `GitHub.Copilot.SDK` 1.0.6 nupkg includes `build/GitHub.Copilot.SDK.props` with `CopilotCliVersion=1.0.69`; do not pin `CopilotCliVersion` in `Directory.Build.props` unless a future package regresses.
