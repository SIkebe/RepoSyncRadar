...
internal static partial class MarkdownPreviewRenderer
{
        ...
        RenderedMarkdownDiffSide diffSide = RenderedMarkdownDiffSide.None)
    {
        ...
        html.AppendLine(".rsr-diff-scrollbar{bottom:0;pointer-events:none;position:fixed;right:2px;top:0;width:7px;z-index:2147483647;}.rsr-diff-scrollbar-marker{border-radius:999px;box-shadow:0 0 0 1px rgba(255,255,255,.7),0 1px 3px rgba(0,0,0,.25);height:4px;min-height:4px;position:absolute;right:0;width:7px;}.rsr-diff-scrollbar-marker--added{background:#2da44e;}.rsr-diff-scrollbar-marker--removed{background:#cf222e;}");
        ...
        html.AppendLine("(() => { const markerRootId = 'rsr-diff-scrollbar'; const selector = '.rsr-rendered-diff-added,.rsr-rendered-diff-removed'; const build = () => { document.getElementById(markerRootId)?.remove(); const elements = Array.from(document.querySelectorAll(selector)); if (elements.length === 0) return; const root = document.scrollingElement || document.documentElement || document.body; const maxScrollTop = Math.max(1, root.scrollHeight - window.innerHeight); const rail = document.createElement('div'); rail.id = markerRootId; rail.className = 'rsr-diff-scrollbar'; elements.forEach(element => { const marker = document.createElement('div'); marker.className = 'rsr-diff-scrollbar-marker ' + (element.classList.contains('rsr-rendered-diff-removed') ? 'rsr-diff-scrollbar-marker--removed' : 'rsr-diff-scrollbar-marker--added'); const top = Math.max(0, Math.min(1, (element.getBoundingClientRect().top + window.scrollY) / maxScrollTop)); marker.style.top = `${(top * 100).toFixed(3)}%`; rail.appendChild(marker); }); document.body.appendChild(rail); }; window.addEventListener('load', build, { once: true }); window.addEventListener('resize', () => window.requestAnimationFrame(build), { passive: true }); })();");
        html.AppendLine("</script>");
        html.AppendLine("</head>");
